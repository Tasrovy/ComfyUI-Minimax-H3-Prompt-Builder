...
编译器会把参考视频中的人物动作或完整表演定义成独立的 `<Subject N>`，通过 `attribute_transfer` 转移给目标人物；只有选择“动作与镜头”或“完整表演”时，`<Video N>` 才另外负责镜头行为和时间结构。选择“完整表演”后，人物卡的默认位置、姿势和表情不会写入该片段，人物实例中明确填写的覆盖值仍然有效。
