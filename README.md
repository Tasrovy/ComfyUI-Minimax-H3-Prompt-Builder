...
采样期间会按照“实时预览间隔（步）”更新低分辨率潜空间动态预览。预览会播放该采样步预测的完整时间序列，并显示当前片段和采样步数；它只用于提前判断动作、镜头和构图，最终画质以 VAE 解码后的成片为准。每个片段完成后，会立即连同音频保存到：
...
当前动作本身严格保持目标秒数和 24 FPS；为了满足 MiniMax H3 的 `17k+5` 帧数规则，只在末尾重复最终姿势补足合法帧数，关联音频的补齐区间使用静音。提示词会为每个 `<Video n>` 和配套 `<Audio n>` 生成正式的媒体定义、保留策略及简短动作职责说明。
