...
动作片段会自动把完整参考动作重定时到自己的开始和结束时间，并将参考视频整理为 MiniMax H3 接受的最接近 `17k+5` 帧数。第一帧和最后一帧都会保留，中间帧按时间均匀抽取或重复；关联音频也会同步到整理后的参考时长。最终提示词会明确写出 `<Video n>` 的实际参考时段与目标动作时段的一一映射，并要求按原顺序保留姿势、轨迹、步法、节奏、重心和接触时刻。
